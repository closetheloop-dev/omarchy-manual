...
<a id="chapter-01-welcome-to-omarchy"></a>
# Welcome to Omarchy!
...
<a id="chapter-02-getting-started"></a>
# Getting Started
...
<a id="chapter-03-coming-from-mac-or-windows"></a>
...
When you genuinely need a floating window, `Super + T` toggles the active one out of the tiling (and back). But give the tiling a real chance first — it's the heart of the whole thing. [Navigation](#chapter-04-navigation) walks you through it.
...
Windows folks: your Win + V clipboard history lives on `Super + Ctrl + V`, and it holds images as well as text. See [unified clipboard & history](#chapter-08-unified-clipboard-history).
...
The instincts transfer faster than you'd think. Skim the [hotkeys](#chapter-07-hotkeys) chapter once, and whenever you blank on a binding, hit `Super + K` — it shows you all of them. That's the only hotkey you actually have to memorize.

<a id="chapter-04-navigation"></a>
# Navigation
...
<a id="chapter-07-hotkeys--tmux"></a>
## Tmux
...
<a id="chapter-07-hotkeys--quick-emojis"></a>
## Quick Emojis
...
<a id="chapter-07-hotkeys--quick-completions"></a>
## Quick Completions
...
<a id="chapter-08-unified-clipboard-history"></a>
...
<a id="chapter-11-text-extraction-dictation"></a>
...
<a id="chapter-12-screenshots-recording"></a>
...
<a id="chapter-13-toggles-idle-screensaver"></a>
...
Inactive indicators are hidden. Hover the area around them and they fade in dimmed, so you can click one to turn it on without knowing its hotkey. Clicking an active one turns it back off. If you'd rather see all of them all the time, set `alwaysShow` to `true` on the `omarchy.indicators` entry in `~/.config/omarchy/shell.json` — see [the top bar](#chapter-05-the-top-bar) for how bar widgets are configured.
...
Omarchy ships with an ergonomically-optimized Tmux configuration, which has a lot of keybindings to learn, so keep [the cheatsheet handy](#chapter-07-hotkeys--tmux).
...
<a id="chapter-18-development-tools"></a>
# Development Tools
...
<a id="chapter-20-shell-functions"></a>
# Shell Functions
...
**Copy URL** puts the current tab's address on your clipboard with `Alt + Shift + L`. That's faster than clicking into the address bar and copying, and because it goes through the system clipboard rather than the browser's, you get an Omarchy notification confirming it and the URL is immediately available in [clipboard history](#chapter-08-unified-clipboard-history) and every other app. There's a toolbar button too, if you prefer clicking.
...
<a id="chapter-24-commercial-apps-services"></a>
...
<a id="chapter-27-filling-out-pdfs"></a>
# Filling out PDFs
...
<a id="chapter-29-other-packages"></a>
# Other Packages
...
<a id="chapter-32-shell-plugins"></a>
# Shell Plugins
...
A full bar plugin has no off state at all. There's always exactly one bar, so you replace it by enabling another one. Bar widget placement is covered in [the top bar](#chapter-05-the-top-bar).
...
<a id="chapter-34-keyboard-mouse-trackpad"></a>
...
By default, Omarchy uses CapsLock as the compose key for [quick emojis](#chapter-07-hotkeys--quick-emojis) and [other completions](#chapter-07-hotkeys--quick-completions). If you'd rather use CapsLock as Caps Lock, move the compose key elsewhere by changing `compose:caps` in `kb_options`. For example, this moves the compose key to Right Alt:
...
<a id="chapter-35-networking"></a>
# Networking

Networking in Omarchy is handled by NetworkManager, and you drive it from the network icon in the [top bar](#chapter-05-the-top-bar) or with `Super + Ctrl + W`.
...
<a id="chapter-36-system-sleep"></a>
# System sleep
...
<a id="chapter-37-hardware-authentication"></a>
...
<a id="chapter-42-common-tweaks"></a>
# Common tweaks
...
<a id="chapter-43-making-your-own-theme"></a>
...
For the sake of this part, we'll assume you've already reviewed [Getting Started](#chapter-02-getting-started) and have your USB drive ready. If you don't go ahead and do that now.
...
4. Proceed with the [install as normal](#chapter-02-getting-started)
...
<a id="chapter-45-troubleshooting"></a>
# Troubleshooting
...
In Omarchy, Caps Lock has been designated to be the xcompose key. That's how you get [quick emojis](#chapter-07-hotkeys--quick-emojis) and [other autocompletions](#chapter-07-hotkeys--quick-completions) done. If you really miss using Caps Lock, you can remap the xcompose key to something else by editing `~/.config/hypr/input.lua`, like setting it to the right alt key:
...
<a id="chapter-47-system-snapshots"></a>
# System snapshots
...
<a id="chapter-50-dual-boot-install"></a>
# Dual Boot Install
...
<a id="chapter-51-unattended-installs"></a>
# Unattended Installs
